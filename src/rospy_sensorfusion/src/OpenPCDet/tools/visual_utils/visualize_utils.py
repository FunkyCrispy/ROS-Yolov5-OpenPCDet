...
def boxes_to_corners_3d(boxes3d):
    ...
    # calculate each border point's offset, and then add the center xyz
...
def draw_corners3d(corners3d, fig, color=(1, 1, 1), line_width=2, cls=None, tag='', max_num=500, tube_radius=None):
    ...
    for n in range(num):
        ...
        # draw 4 lines in each small facet, and 4 lines between them, total 12 lines
        for k in range(0, 4):
            i, j = k, (k + 1) % 4
        ...
        # draw the cross in the closet small facet like a 'x'
        i, j = 0, 5
        mlab.plot3d([b[i, 0], b[j, 0]], [b[i, 1], b[j, 1]], [b[i, 2], b[j, 2]], color=color, tube_radius=tube_radius,
                    line_width=line_width, figure=fig)
        i, j = 1, 4
