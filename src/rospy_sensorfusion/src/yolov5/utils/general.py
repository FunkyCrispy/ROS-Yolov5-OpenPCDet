...
def plot_distance_info(xyxy, img, label, intrinsics_matrix, h, alpha, font_size, color=None):
    ...
    color_ground_point = [60, 20, 250]
    # color_ground_point = (255 - np.array(color)).tolist()
    cv2.circle(img, (x, y), 8, color_ground_point, thickness=-1)

    beta = math.atan((y - v0) / fy)
    gamma = alpha + beta
    if gamma == 0:
        gamma = 0.01

    O2P = round(h / math.tan(gamma), 1)
    z_in_cam = (h / math.sin(gamma)) * math.cos(beta)  # OD
    x_in_cam = z_in_cam * (x - u0) / fx
    y_in_cam = z_in_cam * (y - v0) / fy
    distance = round(math.sqrt(O2P ** 2 + x_in_cam ** 2), 2)  # O2Q，相机光心在地面上的投影点到目标的距离
    # distance = round(math.sqrt(x_in_cam ** 2 + y_in_cam ** 2 + z_in_cam ** 2), 2)  # O2Q，相机光心在地面上的投影点到目标的距离

    if distance < 0:
        distance = "unknown"
    cv2.putText(img, str(distance) + 'm', (x + font_size, y), cv2.FONT_HERSHEY_SIMPLEX, fontScale=font_size, color=color_ground_point,
                thickness=3)
    # print(z_in_cam, math.sin(alpha), math.cos(alpha), (y - v0) / fy, math.cos(alpha) * (y - v0) / fy, math.sin(alpha)+math.cos(alpha) * (y - v0) / fy)
    # print(x, y, np.asarray([x_in_cam, y_in_cam, z_in_cam]))
    # print(z_in_cam, x, y)
    return np.asarray([x_in_cam, y_in_cam, z_in_cam])

#     # label    Q1O     x       y       z        d       a       b       c        atan(b)  O2P
#     # frisbee -18.29 [-5.0675  1.9265  19.042]  18.98   2.7422  0.1008  2.8430  -0.3078  -18.29
#     # bus     -10.18 [ 0.9242 -1.2267  11.571]  10.22   2.7422 -0.1056  2.6366  -0.5529  -10.18
#     # person  -18.25 [ 1.3315  1.9084  18.999]  18.3    2.7422  0.1001  2.8423  -0.3086  -18.25
